...
def perform_ocr(path):
    ...
    try:
        ...
        if not preprocessed_path:
            logger.error(f"Failed to preprocess image at {path}")
            return []
        reader = easyocr.Reader(["en", "ar"], model_storage_directory="model", gpu=False)
        result = reader.readtext(
            preprocessed_path,
            width_ths=1.0,
            height_ths=0.7,
...
def translate_and_replace(path, target_lang):
    ...
    image = Image.fromarray(cv2.cvtColor(clean, cv2.COLOR_BGR2RGB)).convert("RGBA")
    draw = ImageDraw.Draw(image)
    font_path = "static/font/NotoSansArabic-Regular.ttf"  # Arabic-compatible font
    texts_list = []
    is_rtl = target_lang == "ar"  # Check if language is Arabic
    for box, text in boxes:
        if not text:
            continue
        try:
        ...
        if is_rtl:
            trans = get_display(arabic_reshaper.reshape(trans))
        x0, y0 = int(min(p[0] for p in box)), int(min(p[1] for p in box))
        x1, y1 = int(max(p[0] for p in box)), int(max(p[1] for p in box))
        region = cv_img[y0:y1, x0:x1] if (y1 > y0 and x1 > x0) else np.zeros((10,10,3), np.uint8)
        color = choose_contrasting_color(region)
        ...
        font = ImageFont.truetype(font_path, 5)
        lines = []
        # Simplified line breaking for Arabic (no space-based splitting)
        if is_rtl:
            lines = [trans]  # Arabic text is treated as a single line to avoid incorrect splitting
        else:
            words = trans.split()
            current_line = []
            for word in words:
        ...
        for line in lines:
            tw, th = measure_text(draw, line, font)
            # Adjust text alignment for RTL (Arabic)
            if is_rtl:
                pos_x = x0 + (bw - tw)  # Right-align for Arabic
                text_align = 'right'
            else:
                pos_x = x0 + (bw - tw) / 2  # Center for non-RTL
                text_align = 'left'
            draw.text((pos_x, pos_y), line, font=font, fill=color)
            texts_list.append({
                'text': line,
                'left': pos_x,
                'top': pos_y,
                'fontSize': best_size,
                'fill': f'rgb({color[0]},{color[1]},{color[2]})',
                'fontFamily': 'NotoSansArabic' if is_rtl else 'arial',
                'fontWeight': 'normal',
                'fontStyle': 'normal',
                'stroke': None,
                'strokeWidth': 0,
