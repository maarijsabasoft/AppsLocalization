...
#     img = cv2.imread(img_path)  # Load in color to retain potentially useful information
#     if img is None:
#         return None
#     # Apply contrast enhancement on color image
#     img = cv2.convertScaleAbs(img, alpha=1.5, beta=0)  # Increased alpha slightly for better visibility
#     # Use a lighter blur to avoid over-smoothing large text
#     img = cv2.GaussianBlur(img, (3, 3), 0)
#     # Skip adaptive thresholding to prevent artifacts on large text; rely on EasyOCR's internal handling
#     # Increase scale factor for better resolution on small text without harming large text
#     scale_factor = 2.0
...
#             preprocessed_path,
#             width_ths=1.0,  # Increased to merge larger word components
#             height_ths=0.7,
#             mag_ratio=2.0,  # Increased for better recognition accuracy
#             decoder="greedy",
#             min_size=5,
#             text_threshold=0.3,
#             low_text=0.3,  # Added to detect lower-contrast areas
#             batch_size=16
#         )
...
#             if confidence < 0.15:  # Slightly lowered threshold
#                 continue
...
def logout():
    ...
    session.pop('last_image_height', None)
    session.pop('last_fonts', None)
    session.pop('guest_image_count', None)  # Clear guest count on logout
    logout_user()
    return redirect(url_for("login"))
...
def index():
    ...
    if request.method == "POST":
        ...
        if not current_user.is_authenticated:
            session['guest_image_count'] = session.get('guest_image_count', 0)
            if session['guest_image_count'] >= 1:
                flash("Guest users can only process one image. Please log in to continue.", "error")
                return redirect(url_for("login"))
        ...
        # Increment guest image count if not logged in
        if not current_user.is_authenticated:
            session['guest_image_count'] = session.get('guest_image_count', 0) + 1
